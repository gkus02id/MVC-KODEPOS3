var dataAccess = function(){ 
    ...
    this.addItem = function(fieldName,fieldValue,fieldType,fieldKey,flag) {
		if ( typeof fieldName!= "undefined" ) {
		    var newjsonObject = this.jsonObject["dt_objectTable"];
		    var strValue = "";
		    if (fieldType.toLowerCase() == "path") {
		        strValue = coreFilterDataPath(fieldValue);
		    } else {
		        strValue = coreFilterData(fieldValue);
		    }
		    newjsonObject[0]["dt_fieldsCollection"].push({ "fieldName": fieldName, "fieldValue": strValue, "fieldType": fieldType, "fieldKey": fieldKey, "flag": flag });
		}
    }
	this.addItemRelated = function(tableIndex,fieldName,fieldValue,fieldType,fieldKey,flag) {
		if ( typeof fieldName!= "undefined" ) {
			...
			var strRelatedValue = "";
			if (fieldType.toLowerCase() == "path") {
			    strRelatedValue = coreFilterDataPath(fieldValue);
			} else {
			    strRelatedValue = coreFilterData(fieldValue);
			}
			reljsonObject[0]["dt_relfieldCollection"].push({ "fieldName": fieldName, "fieldValue": strRelatedValue, "fieldType": fieldType, "fieldKey": fieldKey, "flag": flag });
		}
    }
