...
// any other els that are already too tall. if `shouldReribute` is on, it considers these tall rows and 
// reduces the available height.
function ributeHeight(els, availableHeight, shouldReribute) {
	...
	unributeHeight(els); // give all elements their natural height
	...
	$(flexEls).each(function(i, el) {
		...
		if (naturalOffset < minOffset) { // we check this again because reribution might have changed things
			$(el).height(newHeight);
		}
	});
}
...
// Undoes rubuteHeight, restoring all els to their natural height
function unributeHeight(els) {
	els.height('');
}
...
function formatRangeWithChunks(date1, date2, chunks, separator, isRTL) {
	...
	// Collect them inctly so we can jam them together later.
...
var DragListener = Class.extend({
	...
	mousedown: function(ev) {
		if (isPrimaryMouseButton(ev)) {
			...
			// start the drag immediately if there is no minimum ance for a drag start
			if (!this.options.ance) {
				this.startDrag(ev);
			}
		}
	},
	...
	mousemove: function(ev) {
		var minDistance;
		var anceSq; // current ance from mouseX0/mouseY0, squared

		if (!this.isDragging) { // if not already dragging...
			// then start the drag if the minimum ance criteria is met
			minDistance = this.options.ance || 1;
			anceSq = Math.pow(ev.pageX - this.mouseX0, 2) + Math.pow(ev.pageY - this.mouseY0, 2);
			if (anceSq >= minDistance * minDistance) { // use pythagorean theorem
				this.startDrag(ev);
			}
		}
	...
	startDrag: function(ev) {
		...
		if (!this.isDragging) {
			...
			// especially important if no min-ance and drag starts immediately
			cell = this.getCell(ev); // this might be different from this.origCell if the min-ance is large
			if (cell) {
				this.cellOver(cell);
			}
		}
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	dayMousedown: function(ev) {
		...
		var dragListener = new DragListener(this.coordMap, {
			//ance: 5, // needs more work if we want dayClick to fire correctly
...
var TimeGrid = Grid.extend({
	...
	slotDuration: null, // duration of a "slot", a inct time segment on given day, visualized by lines
	...
	// This method is inct from 'bg' because we wanted a new `rowType` so the View could customize the rendering.
...
var BasicView = fcViews.basic = View.extend({
	...
	setHeight: function(totalHeight, isAuto) {
		...
		if (eventLimit && typeof eventLimit === 'number') {
			this.dayGrid.limitRows(eventLimit); // limit the levels first so the height can reribute after
		}
	...
	setGridHeight: function(height, isAuto) {
		if (isAuto) {
			unributeHeight(this.dayGrid.rowEls); // let the rows be their natural height with no expanding
		}
		else {
			ributeHeight(this.dayGrid.rowEls, height, true); // true = compensate for height-hogging rows
		}
	},
...
var MonthView = fcViews.month = BasicView.extend({
	...
	setGridHeight: function(height, isAuto) {
		...
		ributeHeight(this.dayGrid.rowEls, height, !isAuto); // if auto, don't compensate for height-hogging rows
	},
