...
namespace afsys.Classes
{
    ...
    {
        public static String getObjectList(SqlConnection oCon, String gridId, String kabupatenId, String format = "html")
        {
            String sqlSelect;
            String urlEdit = "";
            String urlDelete = "";
            String customTextLink = "View";
            String tableId = "dataListKecamatan";
            ...
            try
            {
                sqlSelect = "SELECT ID, NAMA AS KECAMATAN FROM MS_KECAMATAN WITH (NOLOCK) WHERE KABUPATEN_ID = " + kabupatenId + " ORDER BY NAMA";
                ...
                if (dtResult != null)
                {
                    if (dtResult.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow row in dtResult.Rows)
                        {
                            ...
                            urlEdit = "editkecamatan" + "_" + fieldKey;
                            urlDelete = "deletekecamatan_" + fieldKey;
            ...
            {
                Utils.WriteLog("Error Message : \r\n" + ex.Message + "\r\nformat Param : " + format, "Kecamatan", "getObjectList");
            }
            finally
            {
                dtResult.Dispose();
        ...
        public static String Delete(SqlConnection oCon, String id)
        {
            ...
            try
            {
                sqlSelect = "SELECT COUNT(1) AS NRECORD FROM MS_KODEPOS WITH (NOLOCK) WHERE KECAMATAN_ID = " + id;
            ...
            {
                Utils.WriteLog("Error Message : \r\n" + ex.Message + "\r\nformat ID : " + id, "Kecamatan", "Delete");
            }

            if (isValidDelete)
            {
                if (Database.executeQuery(oCon, "DELETE FROM MS_KECAMATAN WHERE ID = " + id, ref message))
                {
                    code = "1";
                }
            }
        ...
        public static KecamatanModels getChoosenObject(SqlConnection oCon, String filterBy)
        {
            KecamatanModels oKecamatan = new KecamatanModels();

            String sqlSelect = "SELECT a.ID, a.NAMA, a.KABUPATEN_ID "
                                + "FROM MS_KECAMATAN AS a WITH (NOLOCK) "
                                + "WHERE a.ID = " + filterBy;
            ...
            using (DataTable oDataTable = Database.doQueryDataTable(oCon, sqlSelect))
            {
                if (oDataTable.Rows.Count > 0)
                {
                    ...
                    {
                        oKecamatan.ID = filterBy;
                        oKecamatan.NAMA = oResult["NAMA"].ToString();
                        oKecamatan.KABUPATEN_ID = oResult["KABUPATEN_ID"].ToString();
                    }
                }
            }
        ...
        public static String getObjectOptions(SqlConnection oCon, String condition = "", String format = "html", String oValue = "", Boolean includeEmptyOption = true)
        {
            ...
            try
            {
                sqlSelect = "SELECT ID, NAMA FROM MS_KECAMATAN WITH (NOLOCK) WHERE KABUPATEN_ID = " + condition + " ORDER BY NAMA";
            ...
            {
                Utils.WriteLog("Error Message : \r\n" + ex.Message +
                               "\r\nformat Param : " + format, "Kecamatan", "getObjectOptions");
            }
